...
for FD in['3']:
    ...
    for i_all_feature_columns in range(len(all_feature_columns)):
        ...
        if __name__ == '__main__':
            ...
            if os.path.exists(last_last_last_path+r"\experiments_result\method_error_txt\{}.txt".format(method_name)):os.remove(last_last_last_path+r"\experiments_result\method_error_txt\{}.txt".format(method_name))
        ...
        #                  verbose=1, validation_data=(X_test, Y_test), callbacks = [reduce_lr,earlystopping,modelcheckpoint])   
                ...
                log.to_excel(last_last_last_path+r"\experiments_result\log\feature_select_valid0\{}_{}_dataset_{}_log{}_time{}.xlsx".format(method_name,all_feature_columns[i_all_feature_columns],dataset,i,datetime.datetime.now().strftime('%Y%m%d%H%M%S')))
